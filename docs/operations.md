...
**実装済みの方針（3アダプタ共通）:** 上記の互換性リスクを踏まえ、実装は素の `UPDATE ... LIMIT` / `DELETE ... LIMIT` 構文を一切使わず、全アダプタ共通で `WHERE rowid IN (SELECT rowid FROM ... WHERE <述語> LIMIT :n)` という移植可能なパターンに統一した（本プロジェクトが使う libSQL ビルドは `SQLITE_ENABLE_UPDATE_DELETE_LIMIT` が無効なため、素の `LIMIT` 付き UPDATE/DELETE はそのままでは動かない）。commit 工程3〜7・観測パージ・UIセッション削除・SyncStaging/SyncSeen パージのいずれもこの rowid サブクエリパターンで実装されている。
...
### 2.3. 初回大量同期時の考慮（実装済みのwindowing範囲）

[sync-protocol.md「Commit 8工程パイプライン」](./sync-protocol.md#commit-8工程パイプライン)の工程3（last_seen確定＋un-stale）〜工程7（Canonical Rollup）は `LIMIT` ウィンドウで分割され、`more:true` が返る間は同一トークンで commit を再送することで続きから処理できる（クラッシュ後の再送も安全に収束）。

一方、工程0（同定採番）〜工程2（Identity生成）はこのウィンドウ分割の対象外で、1回の commit 呼び出しで対象の新規 `external_ref` を全件処理する（D1 の bind パラメータ上限に収まるようバッチINSERTに分割するのみで、複数回の commit 呼び出しに跨る再開の仕組みは持たない）。そのため、極めて大量の新規 `external_ref` を含む初回同期（例: 既存プロジェクトを初めて Discovery 等に接続する場合）は、1回の commit 呼び出しあたりのクエリ数がこの工程0〜2で膨らみうる。

**運用上の推奨:** 初回の大量取り込みは、chunk・sync session を複数回に分割して段階的に実施することを推奨する（1 sync session あたりの新規ケース数を抑える）。工程0〜2 自体への `LIMIT` ウィンドウ導入は MVP では未実装（post-MVP 検討事項）。
...
**`TestCaseHistory` のパージについて（実装済みの事実）:** MVP の `runMaintenance`（本ドキュメント §4.1〜4.3・[data-model.md「TestCaseHistory」](./data-model.md)参照）は `TestCaseObservation` のみをパージ対象とし、`TestCaseHistory` はパージ対象外（post-MVP 検討事項）。`TestCaseHistory` は追記専用（UPDATE/DELETE 禁止）の変更監査ログであり、パージを行うにはこの不変条件と両立する保持ポリシー（例: 何日／何件保持するか、監査要件との整合）を別途決定する必要があるため、MVP では意図的に対象から外している。
...
- 上記の `rowid IN (SELECT rowid ... LIMIT :n)` パターン（§1.3「実装済みの方針」参照）で小バッチ反復して実行する。素の `DELETE...LIMIT N` 構文は使わない。
