...
| `FORBIDDEN` | 403 | role 超過（能力マトリクス上、認証は成功しているが権限が不足。[auth-security.md](./auth-security.md#到達面分離能力マトリクス)参照） |
| `INTERNAL` | 500 | 予期しないサーバーエラー。メッセージは固定文言のみを返し、詳細はサーバー側の構造化ログにのみ記録する |
...
同期プロトコルの `chunk`/`commit` エンドポイントでは `Idempotency-Key` ヘッダを受理する（スペック D-10）。

```
Idempotency-Key: "client-generated-unique-key"
```

**実装済みの事実:** 現状はヘッダを受理するのみで、値の読み取り・メモ化ストアへの保存のいずれも行わない（構造的冪等性のみに依拠する設計）。冪等性は一意制約（`(external_ref, origin, sync_token, fingerprint)` 等）+ `ON CONFLICT DO NOTHING`、および commit 全工程の冪等な WHERE 述語で保証されており、これのみで十分堅牢である。同一キーでの再送に対して前回と完全に同一のレスポンス（例: chunk の `outcome`）が返るとは限らない — たとえば1回目が `inserted`（新規変化点として記録）、再送時が `duplicate`（既存の指紋と一致のため記録省略）になるケースがあるが、これは意味論的に等価であり実害はない。短期メモ化によるレスポンスの完全な同一性保証は、将来の多重防御オプションとして据え置く（現時点では未実装）。
