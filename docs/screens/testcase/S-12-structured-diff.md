...
| 構造化 Diff タブ | tab (active) | `tab-diff` | 現在のタブ。drift 時にバッジ `●` 付き | 常時表示・**常時クリック可能**（スペック D-13-4）。drift 未発生時も disabled/グレーアウトにはしない。クリックすると canonical のみ表示（下記「Diff 不在時」参照） |
...
| エラー | HTTP | code | UI 表示 |
|---|---|---|---|
| OCC 競合 | 409 | `OCC_CONFLICT` | トースト：「更新が競合しました。最新の内容を確認してください」+ 再読み込みボタン |
...
| Accept Fingerprint 失敗（OCC） | error | 「更新が競合しました。最新の内容を確認してください」 |
