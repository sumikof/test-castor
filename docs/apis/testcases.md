...
| updated_at | integer (epoch ms) | 更新日時（D-05: `max(human_updated_at, system_updated_at, created_at)`。未編集・未importでも常に `created_at` 以上の値になる） |

`items` と同じ階層に、同フィルタ条件（カーソル除く）の**正確な件数** `total`（スペック D-03）を含む（[api-reference.md](../api-reference.md#カーソルベースページング)の一般規約「`total` は重い場合は任意/概算」の例外として、テストケース一覧は正確な件数を返す。S-08 の件数表示に使用）。
...
| created_at | integer (epoch ms) | 作成日時 |
| updated_at | integer (epoch ms) | 更新日時（D-05: `max(human_updated_at, system_updated_at, created_at)`。DB列は`human_updated_at`/`system_updated_at`に分離されたまま） |
...
更新可能なフィールドは POST /api/v1/projects/:pid/testcases のリクエスト仕様と同一（すべて任意）**だが、`source_ref` を除く**。`source_ref` は provenance（出所情報）であり、作成時にのみ確定する人間所有列外のフィールドのため、PATCH では値を送っても無視される（`version` の bump・history 記録の対象にもならない）。リクエストボディに `source_ref` を含めること自体はスキーマ上エラーにならないが、実際の更新には反映されない。
...
**位置づけ（スペック D-02）:** `PATCH {status:"archived"}` と同一のドメイン操作（アーカイブ）を実行する**セマンティックエイリアス**。両エンドポイントとも `PATCH /testcases/:id` が正経路で、`DELETE` はそれを呼ぶ別名という位置づけ。
...
なし。`If-Match` ヘッダは不要（下記「業務ルール」参照。PATCH と異なり OCC 非対象）。
...
- **OCC 不要**（`If-Match` を要求しない。`version` 一致条件を課さない UPDATE で実行する）
- **冪等**: 既に `archived` の場合は UPDATE も history 記録も発生させず、現在の状態をそのまま 200 で返す（2重アーカイブがエラーにならない。never 409）
...
| actor_display | string | `actor` の表示名（スペック D-04）。`user:<id>` は `users.display_name`、`token:<id>` は `api_tokens.name` に解決する。削除済み等で解決できなければ `actor` の生値をそのまま入れる |

`items` と同じ階層に、同フィルタ（カーソル除く）の正確な件数 `total` を含む（一覧 API と同様のエンベロープ形状。T13 で受容判断済み）。
...
| updated_at | integer (epoch ms) | 更新日時（D-05: `max(human_updated_at, system_updated_at, created_at)`。算出方法は GET /testcases/:id と共通） |
