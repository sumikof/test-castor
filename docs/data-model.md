...
| human_updated_at | INTEGER (epoch ms) | 人間編集時刻 |
| system_updated_at | INTEGER (epoch ms) | システム書き込み時刻 |
| created_at | INTEGER (epoch ms) | 作成時刻 |
...
- 復帰後もownershipはhumanのまま（machineに戻すにはre-adopt操作が必要）
...
- admin による re-adopt（machine への再採用）操作で誤操作を救済可能
- re-adopt は admin 専用の救済操作。human → machine への逆遷移を許可する。usecase.md 上は明示的なフローとして定義されていないが、誤って human 化したテストケースを Discovery の自動管理下に戻すための非常手段として提供する
...
| SyncSession | `(token, status)` | committed-JOINフェンス |
