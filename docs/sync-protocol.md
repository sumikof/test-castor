...
- `unchanged` — 変化なし
- `staled` — 今回セッションで未出現のため stale マーク
...
今回セッションで観測があった identity の `last_seen_sync_token` を更新し、stale を解除する。
...
WHERE project_id = :P
  AND origin = :O
  AND external_ref IN (
    SELECT DISTINCT o.external_ref
    FROM TestCaseObservation o
    WHERE o.sync_token = :T
  );
```
...
```sql
UPDATE TestCase
SET title = obs.given,  -- observed の内容をマップ
    given = obs.given,
    [when] = obs.[when],
    [then] = obs.[then],
