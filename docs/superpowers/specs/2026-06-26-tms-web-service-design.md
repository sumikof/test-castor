...
importはこのテーブルを物理的に触らない（観測は §4.6 TestCaseObservation へ。Grill Q3）。
...
注：`origin` / `external_ref` / `last_seen_*` は **canonicalから外し**、複数プロデューサが同一ケースを独立同定できるよう §4.5 TestCaseIdentity（マルチホーミング）へ移した（REV-08）。

### 4.5. TestCaseIdentity（衛星ごとの同定・マルチホーミング・REV-08）

1つのcanonicalに対し、**複数のオリジンがそれぞれの `external_ref` で同時に紐づける**。per-origin の同期台帳もここに置くことで、マルチオリジン時のstale判定を正しくする（オリジンXが未観測でもYが観測中なら stale 化しない）。
...
| external_ref | TEXT | 当該オリジンの参照ID（opaque・安定性は衛星責任。Grill Q2）。`(project_id, origin, external_ref)` で一意・冪等同定 |
| last_seen_sync_token | TEXT? | このオリジンの直近syncでの出現トークン（Grill Q6）。stale判定を観測テーブル非走査で高速化 |
| last_seen_at | INTEGER? | このオリジンが直近に出現確認した時刻 |
...
### 4.10. canonical 状態機械（Grill Q1/Q2/Q5）
...
- **stale（Q2/Q5, REV-08）:** sync commit時、当該origin の `TestCaseIdentity` で `last_seen_sync_token != 今回token` のものを未出現とみなし、その canonical を `stale` 化。**ただし他オリジンのidentityが当該ケースを観測中なら stale 化しない**（マルチホーミング）。**approvedは保護**。machine-owned draftの未出現も例外なく `stale` 経由（状態機械を一本化）。
...
- **マルチホーミング（REV-08）**：同定情報（origin/external_ref/last_seen）は `TestCaseIdentity` に分離。1canonicalに複数オリジンが独立同定でき、stale判定は**オリジン別**（あるオリジンが未観測でも他が観測中なら stale 化しない）。
- **import経路の分離（Q3）**：importは `TestCaseObservation`（＋ `TestCaseIdentity.last_seen_*` の軽量update）にのみ書き、canonical本体は触らない。machine-owned時のみcanonicalへミラー昇格。
...
- インデックス: `(project_id, status)`, `(project_id, category)`, Identity側 `(project_id, origin, external_ref)`（一意・同定用）・`(project_id, origin, last_seen_sync_token)`（stale判定用）, Observation側 `(test_case_id, created_at)`・`(project_id, sync_token)`, 生成列索引（REV-11）。
...
| POST | `/api/projects/:pid/sync/:token/chunk` | 観測群を投入。各ケースを `(origin, external_ref)` で `TestCaseIdentity` 同定し、(1) `TestCaseObservation` を**指紋が変化した時のみ**作成（変化点。Q6）、(2) 当該 identity の `last_seen_sync_token`/`last_seen_at` を軽量update、(3) identityが無ければ新規 `draft/machine-owned` canonical＋identityを作成、(4) `machine`-owned canonical は最新観測をミラー、`human`-owned は触れず指紋差を `drift` 記録（Q1/Q3/Q5/Q8/REV-08）。canonical本体の上書きはしない。 |
| POST | `/api/projects/:pid/sync/:token/commit` | セッション確定（1リクエスト内のD1トランザクションでアトミック）。当該origin の identity で `last_seen_sync_token != 今回token` を未出現とみなし、**他オリジンが観測中でない** canonical を `stale` 化（**approved/human-owned approvedは保護**。Q2/Q5/REV-08）。セッションを `committed` に。 |

- 小規模セットでは start→1回 chunk→commit で完結。チャンク分割は大規模時のための任意分割。
- stale判定は `TestCaseIdentity` の `(project_id, origin, last_seen_sync_token)` インデックスで効率化（観測テーブルを走査しない。Q6/REV-08）。
...
3. **テストケース詳細/編集** — Given/When/Thenの構造化フォーム＋`status`変更＋紐づくidentity（origin/external_ref）/`source_ref`表示＋「Gherkinビュー」タブ＋変更履歴。**`drift` バッジ**（human-ownedで観測と乖離）と **`stale` バッジ**（コード側で消えた候補）を表示し、人間が確認・整合できる導線を置く（Grill Q1/Q2）。
...
- **共通（必須要件）:** 保持ポリシー（直近N変化点／T日）に基づく**定期パージ**。クラウドは **Cloudflare Cron Triggers の scheduled Worker**、オンプレは cron/スケジューラで、`Storage` 裏の同一パージ操作を呼ぶ。
- **D1（クラウド）:** D1は **`VACUUM`/`auto_vacuum` を公開PRAGMAに持たず**、物理リクラメーションを**自前管理**する（容量超過時の公式対処も「行DELETE or DB分割(shard)」）。よってD1側のレバーは **定期パージ＋（大規模時）テナント別D1へのshard**（Grill Q6）であり、VACUUMは用いない。
