...
- **`src/storage/adapters/better-sqlite3.ts`** に `busy_timeout` プラグマ追加(+ revert-proof 単体テスト)— DC-07
- スモーク検証スクリプト(受け入れ確認用)
...
### DC-07 SQLite 並行性: `busy_timeout` 追加(src 変更・承認済み)

web と maintenance が**同一 SQLite ファイル(WAL)へ並行書込み**する。この並行性は README 記載の既存オンプレ運用(web サーバ + OS cron の maintenance-cli)にも既に存在するが、`better-sqlite3` の既定 `busy_timeout` は 0 で、稀に `SQLITE_BUSY` を返し得る。

- 対応: `createBetterSqlite3Storage` で `sqlite.pragma('busy_timeout = 5000')` を設定(ロック競合時に最大 5 秒待つ)。
- プラグマ順序: `busy_timeout` は「`auto_vacuum` → WAL の順」制約(HANDOVER §5-8)とは独立。安全のため **DB オープン直後**(`foreign_keys = ON` の近辺、`auto_vacuum`/WAL 設定より前)に発行する。
- テスト(revert-proof): 構築後に `PRAGMA busy_timeout` が 5000 を返すことを assert する単体テストを追加。プラグマ行を削除すると 0 になり落ちる(識別的)。3 アダプタ共有契約テストは内部設定のため不変。
- 値の env 化(`SQLITE_BUSY_TIMEOUT_MS` 等)は YAGNI。将来必要になれば追加。
...
- **変更**: `src/storage/adapters/better-sqlite3.ts`(`busy_timeout` 1 行 — DC-07)、`tests/unit/*`(busy_timeout テスト)、`.gitignore`(`.env` 追加)、`README.md`(Docker セクション追加)
...
6. **回帰**: `busy_timeout` 単体テスト green、`npm test`(815+)green、`npm run typecheck` 0 エラー
