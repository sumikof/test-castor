...
describe('middleware/authn: 振る舞い表(auth-security.md 能力マトリクス・IDOR 404/403)', () => {
  ...
  describe('セッション認証: Cookie なし / 署名不正 / DB に無い / expires_at<=now → 401(失効行は削除)', () => {
    ...
    it('署名不正(改竄された署名) → 401', async () => {
      ...
      // 署名(base64url)の"末尾1文字"を反転する実装は使わない: 32B HMAC-SHA256 の出力は
      // 32 mod 3 = 2 の端数を持つため、base64url エンコードの最終文字は実データ4bit+デコード時に
      // 破棄される2bitのパディングしか担わない。atob はパディングbitの整合性を検査しないため、
      // 末尾文字がたまたま 'A' だった場合(パディングbitを無視すれば 'B' と同一のデコード結果になる)、
      // "改竄" がデコード後のバイト列を全く変えず検証をすり抜けてしまう(実測で約1/16の確率で発生。
      // これは cross-file のグローバル汚染ではなく本テスト自身の改竄ロジックの欠陥だった)。
      // 署名の"先頭"文字は常に完全な6bitを担い、パディングの影響を受けない(auth-session-sign.test.ts
      // 「改竄: 署名部分の1文字を書き換えると null」と同じ根拠)ため、先頭文字を反転して
      // 確率に依存せず決定的に署名を無効化する。
      const sigStart = header.lastIndexOf('.') + 1;
      const firstSigChar = header.charAt(sigStart);
      const tampered =
        header.slice(0, sigStart) + (firstSigChar === 'A' ? 'B' : 'A') + header.slice(sigStart + 1);
